...
public class Vec3
{
    public static Vec3 Zero = new(0f);
    ...
    {
        float sinHalfAngle = (float)System.Math.Sin((angle / 2).ToRadians());
        float cosHalfAngle = (float)System.Math.Cos((angle / 2).ToRadians());
    ...
    public Vec3 Abs()
    {
        return new Vec3(System.Math.Abs(X), System.Math.Abs(Y), System.Math.Abs(Z));
    }
